...
const Transactions = (() => {
  ...
  function openModal(id = null, presetType = null) {
    editingId = id;
    ...
    Utils.byId("txAccount").innerHTML = Accounts.accountOptionsHTML(tx ? tx.accountId : "");
    ...
    return `<tr>
      ...
      <td>
        <div class="row-actions">
          <button class="btn btn-icon btn-ghost btn-sm" data-edit-tx="${t.id}" aria-label="Edit">${icon("settings")}</button>
          <button class="btn btn-icon btn-ghost btn-sm" data-delete-tx="${t.id}" aria-label="Delete">${icon("x")}</button>
        </div>
      </td>
    </tr>`;
  }
  ...
  function recentForDashboard(limit = 5) {
    return applyFilters(getAll()).slice(0, limit);
  }
