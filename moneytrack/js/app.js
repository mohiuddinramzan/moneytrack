...
const App = (() => {
  ...
  function renderBottomNav() {
    const primary = ["dashboard", "transactions", "investments", "reports", "settings"];
  ...
  function renderDashboard() {
    ...
    const goalsList = Utils.byId("goalsProgressList");
    goalsList.innerHTML = s.goals.length
      ? ""
      : `<div class="empty-state">${icon("target", "icon")}<div class="empty-state__title">No goals yet</div><p>Set a savings target to track your progress.</p></div>`;
  }
  ...
  function initRoutes() {
    ...
    Router.register("debts", () => {});
    Router.register("goals", () => {});
    Router.register("reports", () => {});
    Router.register("settings", () => {});
  }

  // Called by other modules after they mutate data, so the currently
  // visible view (which may show derived data) stays in sync.
  function refreshDependents() {
    const route = Router.getCurrentRoute();
    if (route === "dashboard") renderDashboard();
    else if (route === "accounts") Accounts.render();
    else if (route === "transactions") Transactions.render();
    else if (route === "investments") Investments.render();
  }
