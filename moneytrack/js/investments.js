...
const Investments = (() => {
  ...
  function renderAllocationChart(allocation) {
    const chartEl = Utils.byId("allocationChart");
    const legendEl = Utils.byId("allocationLegend");
    ...
    return `<tr>
      ...
      <td>
        <div class="row-actions">
          <button class="btn btn-icon btn-ghost btn-sm" data-edit-inv="${inv.id}" aria-label="Edit">${icon("settings")}</button>
          <button class="btn btn-icon btn-ghost btn-sm" data-delete-inv="${inv.id}" aria-label="Delete">${icon("x")}</button>
        </div>
      </td>
    </tr>`;
  }
  ...
  return { init, render, openModal, getAll, typeLabel, summary, recentForDashboard };
})();
